...
#define WD_CLIENT_ASYNC_TCP_DEFAULT_PENDING_BYTES    (1024ull * 1024ull)
#define WD_CLIENT_ASYNC_UDP_DEFAULT_RING_ENTRIES     256u
...
#define WD_CLIENT_TCP_TX_RING_ENTRIES                64u
#define WD_CLIENT_TCP_TX_PENDING_BYTES               (1024ull * 1024ull)
#define WD_CLIENT_UDP_RX_RING_ENTRIES                256u
...
#define WD_NET_LINK_PROBE_COUNT                      5u
...
#define WD_AUDIO_TX_RING_ENTRIES                    32u
...
#define WD_WIRE_TILE_MAX_BASE_TILES \
...
WD_CONFIG_STATIC_ASSERT(WD_DEFAULT_TCP_PORT > 0u && WD_DEFAULT_TCP_PORT <= UINT16_MAX,
                        "default TCP port must fit uint16_t");
WD_CONFIG_STATIC_ASSERT(WD_ASYNC_MIN_RING_ENTRIES > 0u &&
                        WD_SERVER_CONTROL_TX_RING_ENTRIES >= WD_ASYNC_MIN_RING_ENTRIES &&
...
WD_CONFIG_STATIC_ASSERT(WD_SERVER_ASYNC_UDP_PENDING_MIN_PACKETS <=
                        WD_SERVER_ASYNC_UDP_PENDING_MAX_PACKETS,
                        "server UDP pending bounds must be ordered");
WD_CONFIG_STATIC_ASSERT(WD_NET_MTU_PROBE_PAYLOAD_FLOOR >= WD_MIN_PROBED_UDP_PAYLOAD,
                        "MTU payload floor must satisfy the accepted minimum");
WD_CONFIG_STATIC_ASSERT(WD_SERVER_MIN_OUTPUT_SCALE_MILLI > 0u &&
                        WD_SERVER_MIN_OUTPUT_SCALE_MILLI <= WD_SERVER_DEFAULT_OUTPUT_SCALE_MILLI &&
...
WD_CONFIG_STATIC_ASSERT(WD_STREAM_TILE_COMPRESSION_MIN_SAVINGS_PERCENT <= 100u &&
                        WD_STREAM_VIDEO_DERIVED_BUDGET_PERCENT <= 100u &&
                        WD_TILE_AUTO_ENTRY_CHANGED_FRAMES_PERCENT <= 100u &&
                        WD_TILE_AUTO_ENTRY_WIRE_PRESSURE_PERCENT <= 100u &&
...
WD_CONFIG_STATIC_ASSERT(WD_WIRE_TILE_MAX_WIDTH % WD_BASE_TILE_WIDTH == 0u &&
                        WD_WIRE_TILE_MAX_HEIGHT % WD_BASE_TILE_HEIGHT == 0u &&
                        WD_WIRE_TILE_MAX_BASE_TILES > 0u &&
                        WD_WIRE_TILE_MAX_BASE_TILES <= UINT16_MAX,
                        "wire-tile coverage must fit base-tile storage");
WD_CONFIG_STATIC_ASSERT(WD_TILE_SIZE_LARGE_WIDTH == WD_WIRE_TILE_MAX_WIDTH &&
                        WD_TILE_SIZE_LARGE_HEIGHT == WD_WIRE_TILE_MAX_HEIGHT &&
                        WD_TILE_SIZE_BASE_WIDTH == WD_BASE_TILE_WIDTH &&
                        WD_TILE_SIZE_BASE_HEIGHT == WD_BASE_TILE_HEIGHT,
                        "supported tile endpoints must match protocol geometry");
WD_CONFIG_STATIC_ASSERT(WD_AUDIO_TARGET_LATENCY_MS_MIN <=
...
WD_CONFIG_STATIC_ASSERT(WD_AUDIO_SAMPLE_RATE_DEFAULT > 0u &&
...
WD_CONFIG_STATIC_ASSERT(WD_VIDEO_MIN_DIRTY_PERCENT_DEFAULT <=
                        WD_VIDEO_MIN_DIRTY_PERCENT_MAX &&
                        WD_VIDEO_EXIT_DIRTY_PERCENT_DEFAULT <= WD_VIDEO_EXIT_DIRTY_PERCENT_MAX &&
                        WD_VIDEO_MIN_DIRTY_PERCENT_MAX <= 100u &&
                        WD_VIDEO_EXIT_DIRTY_PERCENT_MAX <= 100u,
...
WD_CONFIG_STATIC_ASSERT(WD_CLIENT_FRAMEBUFFER_LOCK_EWMA_OLD_NUMERATOR <
                        WD_CLIENT_FRAMEBUFFER_LOCK_EWMA_DENOMINATOR,
                        "framebuffer lock EWMA must retain some new samples");
WD_CONFIG_STATIC_ASSERT(WD_SERVER_KEY_QUEUE_CAPACITY > 0u &&
                        WD_SERVER_POINTER_QUEUE_CAPACITY > 0u &&
                        WD_SERVER_PRESSED_KEY_CAPACITY > 0u,
                        "input queue capacities must be nonzero");
WD_CONFIG_STATIC_ASSERT(WD_SERVER_PROCESS_TERM_GRACE_MS > 0u &&
                        WD_SERVER_PROCESS_KILL_GRACE_MS > 0u &&
                        WD_SERVER_PROCESS_POLL_INTERVAL_MS > 0u,
                        "process shutdown timing must be bounded");
WD_CONFIG_STATIC_ASSERT(WD_SERVER_MIN_REFRESH_HZ > 0u &&
                        WD_SERVER_MIN_REFRESH_HZ <= WD_SERVER_DEFAULT_REFRESH_HZ &&
...
WD_CONFIG_STATIC_ASSERT(WD_DISPLAY_WIDTH <= WD_MAX_RENDER_WIDTH &&
                        WD_DISPLAY_HEIGHT <= WD_MAX_RENDER_HEIGHT,
                        "default display must fit the render envelope");
WD_CONFIG_STATIC_ASSERT(WD_CLIENT_DIRTY_RECT_FULL_UPLOAD_PERCENT <= 100u &&
                        WD_CLIENT_BOUNDS_UPLOAD_MIN_SAVINGS_PERCENT <= 100u &&
                        WD_CLIENT_FULL_UPLOAD_MIN_SAVINGS_PERCENT <= 100u,
                        "render percentages must be valid");
WD_CONFIG_STATIC_ASSERT(WD_LIMITED_MODE_THROUGHPUT_SAFETY_PERCENT <= 100u &&
                        WD_LINK_SUMMARY_BUDGET_PERCENT <= 100u &&
                        WD_CLIENT_REPAIR_PRESSURE_PERCENT <= 100u,
                        "network percentages must be valid");
WD_CONFIG_STATIC_ASSERT(WD_LIMITED_MODE_MIN_UDP_BYTES_PER_SECOND <=
                        WD_LIMITED_MODE_DEFAULT_UDP_BYTES_PER_SECOND &&
                        WD_LIMITED_MODE_DEFAULT_UDP_BYTES_PER_SECOND <=
                        WD_LIMITED_MODE_MAX_UDP_BYTES_PER_SECOND,
                        "limited-mode bandwidth defaults must be ordered");
WD_CONFIG_STATIC_ASSERT(WD_CLIENT_VIDEO_METADATA_QUEUE_CAPACITY >=
                        WD_CLIENT_VIDEO_DECODED_QUEUE_CAPACITY &&
                        WD_CLIENT_VIDEO_DECODED_QUEUE_CAPACITY >=
                        WD_CLIENT_VIDEO_PRESENT_QUEUE_CAPACITY,
                        "video queue capacities must be ordered");
WD_CONFIG_STATIC_ASSERT(WD_AUDIO_CAPTURE_RING_MS >= WD_AUDIO_TX_QUEUE_MS,
                        "audio capture buffering must cover the transmit queue");
...
WD_CONFIG_STATIC_ASSERT(WD_VIDEO_ENCODER_FALLBACK_FPS > 0u &&
                        WD_VIDEO_ENCODER_SOFTWARE_THREADS > 0u &&
                        WD_VIDEO_ENCODER_GOP_SECONDS > 0u,
                        "video encoder defaults must be nonzero");
WD_CONFIG_STATIC_ASSERT(WD_CLIENT_RENDER_COST_EWMA_OLD_NUMERATOR <
