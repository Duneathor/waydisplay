...
static inline uint64_t wd_audio_reserve_from_tile_budget(uint64_t measured_bytes_per_second, uint32_t codec_bitrate) {
    const uint64_t reserve = wd_audio_reserved_bytes_per_second(codec_bitrate);
    if (reserve == 0 || measured_bytes_per_second == 0)
