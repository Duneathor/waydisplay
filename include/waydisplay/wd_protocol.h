...
#define WD_VIDEO_FRAME_FLAG_MASK (WD_VIDEO_FRAME_CONFIG | WD_VIDEO_FRAME_KEYFRAME | WD_VIDEO_FRAME_RESIZE | WD_VIDEO_FRAME_END_OF_STREAM)
...
struct wd_client_hello_payload {
    ...
    /*
     * Requested session cadence. 0 means the product default. The server
...
enum wd_client_audio_playback_state {
    ...
    WD_CLIENT_AUDIO_PLAYBACK_BUFFERING = 1,
    WD_CLIENT_AUDIO_PLAYBACK_PLAYING   = 2,
    WD_CLIENT_AUDIO_PLAYBACK_STARVED   = 3,
};
...
struct wd_client_stats_payload {
    ...
    uint64_t tile_content_epoch_presented;
    uint64_t video_content_epoch_presented;
    uint64_t video_decode_queue_drops;
...
static inline uint16_t wd_udp_tile_header_size_for_flags(uint8_t flags) {
    ...
    return (uint16_t)(sizeof(struct wd_udp_tile_packet_header) +
                      ((flags & WD_UDP_TILE_FLAG_INPUT_SEQUENCE) != 0 ? sizeof(struct wd_udp_tile_input_sequence_extension) : 0));
}
...
static inline bool wd_udp_tile_packet_encode_header(void* destination, size_t destination_size,
    ...
    if (!destination || !header || header->session_id == 0 || header->connection_token == 0 || header->content_epoch == 0 ||
        header->tile_pkt_count == 0 || header->tile_pkt_id >= header->tile_pkt_count || header->payload_size == 0 ||
        header->tile_payload_size == 0 || header->payload_size > header->tile_payload_size || header->tile_generation == 0 ||
    ...
    if (header_size == 0 || destination_size < header_size ||
        ((header->flags & WD_UDP_TILE_FLAG_INPUT_SEQUENCE) != 0 && (header->tile_pkt_id != 0 || header->input_sequence == 0)))
    {
        return false;
    }
...
static inline bool wd_udp_tile_packet_decode(const void* packet, size_t packet_size, struct wd_udp_tile_packet_decoded* out) {
    ...
    uint16_t       tile_width  = 0;
    uint16_t       tile_height = 0;
    if (header_size == 0 || packet_size < header_size || wire.session_id == 0 || wire.connection_token == 0 || wire.content_epoch == 0 ||
        wire.reserved != 0 || wire.tile_pkt_count == 0 || wire.tile_pkt_id >= wire.tile_pkt_count || wire.payload_size == 0 ||
        wire.tile_payload_size == 0 || wire.payload_size > wire.tile_payload_size || wire.tile_generation == 0 ||
...
static inline bool wd_udp_tile_fragment_layout_valid(const struct wd_udp_tile_packet_decoded* header, size_t packet_size,
                                                     uint16_t udp_payload_target, uint32_t total_payload_size) {
    if (!header || udp_payload_target == 0 || total_payload_size == 0 || total_payload_size != header->tile_payload_size)
    {
        return false;
    }
    ...
    const uint32_t expected_packet_count = (total_payload_size + (uint32_t)udp_payload_target - 1u) / (uint32_t)udp_payload_target;
    if (expected_packet_count == 0 || expected_packet_count > UINT8_MAX || header->tile_pkt_count != expected_packet_count ||
        header->tile_pkt_id >= expected_packet_count)
    {
        return false;
    }
    ...
    const uint16_t expected_payload_size = (uint16_t)(remaining > udp_payload_target ? udp_payload_target : remaining);
...
#define WD_IPV4_MTU_TO_TILE_PAYLOAD(mtu) ((uint16_t)((mtu) - WD_IPV4_HEADER_BYTES - WD_UDP_HEADER_BYTES - WD_UDP_TILE_HEADER_MAX_SIZE))
...
static inline bool wd_config_applied_matches(const struct wd_config_applied_payload* applied, uint8_t session_id, uint64_t connection_token,
                                             uint64_t config_epoch) {
    return applied && session_id != 0 && connection_token != 0 && config_epoch != 0 && applied->session_id == session_id &&
           applied->connection_token == connection_token && applied->config_epoch == config_epoch;
}
...
static inline bool wd_keyboard_event_payload_is_valid(const struct wd_keyboard_event_payload* key, uint32_t payload_size) {
    return key && wd_fixed_payload_size_is_valid(payload_size, sizeof(*key)) && key->session_id != 0 && key->connection_token != 0 &&
           key->client_timestamp_ns != 0 && key->input_sequence != 0 && key->evdev_key_code != 0 && key->pressed <= 1;
}

static inline bool wd_pointer_event_payload_is_valid(const struct wd_pointer_event_payload* pointer, uint32_t payload_size) {
    if (!pointer || !wd_fixed_payload_size_is_valid(payload_size, sizeof(*pointer)) || pointer->session_id == 0 ||
        pointer->connection_token == 0 || pointer->client_timestamp_ns == 0 || pointer->input_sequence == 0 ||
        (pointer->modifiers & ~0x0fu) != 0)
    {
        return false;
    }
    ...
    case WD_POINTER_EVENT_MOTION:
        return pointer->button == 0 && pointer->button_state == 0 && pointer->axis == 0 && pointer->axis_value == 0;
    case WD_POINTER_EVENT_BUTTON:
        return pointer->button != 0 && pointer->button_state <= WD_POINTER_BUTTON_PRESSED && pointer->axis == 0 && pointer->axis_value == 0;
    case WD_POINTER_EVENT_AXIS:
        return pointer->button == 0 && pointer->button_state == 0 && pointer->axis <= WD_POINTER_AXIS_HORIZONTAL &&
...
static inline bool wd_counted_payload_size_is_valid(uint32_t payload_size, size_t header_size, uint32_t count, size_t entry_size) {
...
static inline bool wd_selection_payload_size_is_valid(const struct wd_selection_payload_header* header, uint32_t payload_size) {
...
static inline bool wd_tile_summary_count_is_valid(uint16_t flags, uint16_t tile_count, uint16_t total_tiles) {
    return total_tiles != 0 && tile_count <= total_tiles && ((flags & WD_TILE_SUMMARY_DELTA) != 0 || tile_count == total_tiles);
}
...
static inline bool wd_client_hello_payload_is_valid(const struct wd_client_hello_payload* hello, uint32_t payload_size) {
    if (!hello || !wd_fixed_payload_size_is_valid(payload_size, sizeof(*hello)) || (hello->capabilities & ~WD_CLIENT_CAP_MASK) != 0 ||
        (hello->video_codecs & ~WD_VIDEO_CODEC_MASK) != 0 || hello->video_reserved != 0 ||
        (hello->audio_codecs & ~WD_AUDIO_CODEC_MASK) != 0 || hello->audio_reserved != 0 || hello->video_mode > WD_VIDEO_MODE_FORCE ||
        hello->video_min_dirty_percent > WD_VIDEO_MIN_DIRTY_PERCENT_MAX ||
        hello->video_exit_dirty_percent > WD_VIDEO_EXIT_DIRTY_PERCENT_MAX || hello->video_enter_seconds > WD_VIDEO_ENTER_SECONDS_MAX ||
        hello->video_exit_seconds > WD_VIDEO_EXIT_SECONDS_MAX || ((hello->desired_width == 0) != (hello->desired_height == 0)) ||
    ...
    if (audio)
    {
        return hello->audio_codecs != 0 && hello->audio_transport == WD_AUDIO_TRANSPORT_TCP && hello->audio_max_channels != 0 &&
               hello->audio_max_channels <= WD_AUDIO_CHANNELS_MAX &&
               (hello->audio_target_latency_ms == 0 || (hello->audio_target_latency_ms >= WD_AUDIO_TARGET_LATENCY_MS_MIN &&
                                                        hello->audio_target_latency_ms <= WD_AUDIO_TARGET_LATENCY_MS_MAX));
    }
    return hello->audio_codecs == 0 && hello->audio_transport == 0 && hello->audio_max_channels == 0 && hello->audio_target_latency_ms == 0;
}
...
static inline bool wd_audio_config_payload_is_valid(const struct wd_audio_config_payload* config, uint32_t payload_size) {
    return config && wd_fixed_payload_size_is_valid(payload_size, sizeof(*config)) && config->session_id != 0 &&
           config->connection_token != 0 && config->audio_epoch != 0 && config->media_clock_id != 0 &&
           config->codec == WD_AUDIO_CODEC_OPUS && config->sample_rate == WD_AUDIO_SAMPLE_RATE_DEFAULT && config->channels != 0 &&
           config->channels <= WD_AUDIO_CHANNELS_MAX && config->reserved == 0 && wd_audio_frame_samples_is_valid(config->frame_samples) &&
           config->codec_delay_samples <= config->frame_samples && config->target_bitrate != 0;
}

static inline bool wd_audio_packet_payload_size_is_valid(const struct wd_audio_packet_payload_header* header, uint32_t tcp_payload_size) {
    if (!header || tcp_payload_size < sizeof(*header) || header->session_id == 0 || header->connection_token == 0 ||
        header->audio_epoch == 0 || header->media_clock_id == 0 || header->sequence == 0 ||
        (header->flags & ~WD_AUDIO_PACKET_FLAG_MASK) != 0)
    {
        return false;
    }
    ...
    {
        return header->flags == WD_AUDIO_PACKET_END_OF_STREAM && header->data_size == 0 && header->duration_samples == 0;
    }

    return header->data_size != 0 && wd_audio_frame_samples_is_valid(header->duration_samples);
}

static inline bool wd_video_frame_payload_size_is_valid(const struct wd_video_frame_payload_header* header, uint32_t tcp_payload_size) {
    if (!header || tcp_payload_size < sizeof(*header) || header->session_id == 0 || header->connection_token == 0 ||
        header->content_epoch == 0 || (header->flags & ~WD_VIDEO_FRAME_FLAG_MASK) != 0 || header->codec == 0 ||
        (header->codec & ~WD_VIDEO_CODEC_MASK) != 0 || (header->codec & (header->codec - 1u)) != 0)
    {
        return false;
    }
    ...
    if (control)
    {
        return header->data_size == 0 && header->frame_id == 0 && header->width != 0 && header->height != 0 &&
               header->coded_width == header->width && header->coded_height == header->height && (!resize || end_of_stream) &&
    ...
    return header->data_size != 0 && header->frame_id != 0 && header->width != 0 && header->height != 0 &&
           ...
           ((header->flags & WD_VIDEO_FRAME_CONFIG) == 0 || (header->flags & WD_VIDEO_FRAME_KEYFRAME) != 0);
}
