...
void test_tile_count_boundaries() {
    ...
    require(wd_tiles_for_width_with_tile(UINT32_MAX, 1) == 0,
            "tile counts beyond the wire ID range should fail closed");
    require(wd_total_tiles_for_size_with_tile(4096, 4096, 1, 1) == 0,
            "total tile count beyond uint16 should be rejected");
    require(wd_total_tiles_for_size_with_tile(0, 1, 1, 1) == 0,
            "zero dimension should produce no tile grid");
    ...
    require(wd_total_tiles_for_size(17, 17) ==
                wd_total_tiles_for_size_with_tile(17, 17, WD_TILE_WIDTH, WD_TILE_HEIGHT),
            "default total helper should delegate to configured tile size");
}

void test_tile_coordinates_and_visible_edges() {
    require(wd_tile_x_for(7, 0) == 0 && wd_tile_y_for(7, 0) == 0,
            "zero-column grid should not divide by zero");
    require(wd_tile_x_for(3, 2) == 1 && wd_tile_y_for(3, 2) == 1,
            "tile ID should map to row-major coordinates");
    require(wd_tile_start_x_for_tile(3, 2, 4) == 4 &&
                wd_tile_start_y_for_tile(3, 2, 2) == 2,
            "tile start should scale row-major coordinates");
    require(wd_tile_visible_width_for_tile(5, 3, 2, 4) == 1,
            "right edge tile should expose only visible width");
    require(wd_tile_visible_height_for_tile(3, 3, 2, 2) == 1,
            "bottom edge tile should expose only visible height");
    require(wd_tile_visible_width_for_tile(4, 3, 2, 4) == 0,
            "fully offscreen tile should have zero visible width");
    require(wd_tile_visible_height_for_tile(2, 3, 2, 2) == 0,
            "fully offscreen tile should have zero visible height");
    require(wd_tile_id_valid_for(3, 4) && !wd_tile_id_valid_for(4, 4),
            "tile ID validation should be half-open");

    require(wd_tile_id_valid(0), "default first tile should be valid");
    require(wd_tile_x(1) == 1 && wd_tile_y(WD_TILES_X) == 1,
            "default coordinate wrappers should remain row-major");
...
void test_extract_hash_and_blit_partial_tiles() {
    ...
    require(wd_extract_tile_xrgb8888_for_tile(source.data(), width, height, tiles_x, total_tiles,
                                               3, tile_width, tile_height, tile.data()),
    ...
    require(wd_blit_tile_xrgb8888_for_tile(destination.data(), width, height, tiles_x, total_tiles,
                                            3, tile_width, tile_height, tile.data()),
    ...
    const uint32_t hash_before = wd_fnv1a_tile_hash_xrgb8888_for_tile(
        source.data(), width, height, tiles_x, total_tiles, 0, tile_width, tile_height);
    source[0] ^= 1u;
    const uint32_t hash_after = wd_fnv1a_tile_hash_xrgb8888_for_tile(
        source.data(), width, height, tiles_x, total_tiles, 0, tile_width, tile_height);
    require(hash_before != 0 && hash_after != hash_before, "tile hash should reflect visible pixel changes");

    require(wd_fnv1a_tile_hash_xrgb8888_for_tile(nullptr, width, height, tiles_x, total_tiles,
                                                  0, tile_width, tile_height) == 0,
            "hash should reject null framebuffer");
    require(wd_fnv1a_tile_hash_xrgb8888_for_tile(source.data(), width, height, tiles_x, total_tiles,
                                                  total_tiles, tile_width, tile_height) == 0,
            "hash should reject invalid tile ID");
    require(!wd_extract_tile_xrgb8888_for_tile(nullptr, width, height, tiles_x, total_tiles,
                                                0, tile_width, tile_height, tile.data()),
            "extract should reject null framebuffer");
    require(!wd_extract_tile_xrgb8888_for_tile(source.data(), width, height, tiles_x, total_tiles,
                                                0, 0, tile_height, tile.data()),
            "extract should reject zero tile width");
    require(!wd_blit_tile_xrgb8888_for_tile(destination.data(), width, height, tiles_x, total_tiles,
                                             0, tile_width, tile_height, nullptr),
...
void test_zstd_one_shot_and_context_contracts() {
    ...
    require(wd_zstd_compress(source.data(), source.size(), compressed.data(), compressed.size(), 1,
                             &compressed_size),
    ...
    compressed_size = 77;
    require(wd_zstd_compress_with_context(context, source.data(), source.size(), compressed.data(), compressed.size(),
                                           3, &compressed_size),
    ...
    compressed_size = 55;
    require(!wd_zstd_compress(nullptr, source.size(), compressed.data(), compressed.size(), 1, &compressed_size) &&
                compressed_size == 0,
            "invalid one-shot input should fail and clear output size");
    compressed_size = 55;
    require(!wd_zstd_compress(source.data(), source.size(), compressed.data(), 1, 1, &compressed_size) &&
                compressed_size == 0,
            "insufficient output capacity should fail cleanly");
    compressed_size = 55;
    require(!wd_zstd_compress_with_context(nullptr, source.data(), source.size(), compressed.data(), compressed.size(),
                                            1, &compressed_size) && compressed_size == 0,
            "null context should fail and clear output size");
    require(!wd_zstd_decompress(nullptr, 1, decoded.data(), decoded.size(), source.size()),
            "decompression should reject null source");
