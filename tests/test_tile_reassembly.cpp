...
std::vector<std::vector<uint8_t>> make_packets(const ClientState& state, uint8_t tile_size, uint16_t tile_id, uint64_t generation,
                                               const std::vector<uint8_t>& tile_bytes, bool compressed, bool latency) {
    std::vector<uint8_t> wire_payload = tile_bytes;
    if (compressed)
    {
        std::vector<uint8_t> output(wd_zstd_compress_bound(tile_bytes.size()));
        uint32_t             output_size = 0;
        require(wd_zstd_compress(tile_bytes.data(), tile_bytes.size(), output.data(), output.size(), 1, &output_size), "test compression");
        output.resize(output_size);
        wire_payload = std::move(output);
    }

    const uint16_t target           = state.config.udp_payload_target;
    const uint32_t packet_count_u32 = (static_cast<uint32_t>(wire_payload.size()) + target - 1u) / target;
...
void test_large_tile_marks_all_covered_repairs_inflight() {
    ...
    for (uint16_t base_id : std::array<uint16_t, 4>{0, 1, 4, 5})
    {
        require(state.retx_inflight_generation[base_id] == 6, "every base tile covered by a large tile should be marked in flight");
...
void test_newer_generation_replaces_covered_inflight_state() {
    ...
    for (uint16_t base_id : std::array<uint16_t, 4>{0, 1, 4, 5})
    {
        require(state.retx_inflight_generation[base_id] == 7, "newer large-tile generation should replace covered inflight state");
    }
}
...
void test_out_of_order_and_duplicate_fragments() {
    ...
    require(!reassembler.process_udp_packet(state, packets[0].data(), packets[0].size()).valid, "two unique packets should remain partial");
...
void test_large_tile_accepts_mixed_equal_and_older_coverage() {
    ...
    require(completed.valid && completed.tile_bytes == bytes, "large tile should repair base tiles that are still older");
}
...
void test_reuses_bounded_reassembly_storage() {
    ...
    require(!reassembler.process_udp_packet(state, packets_b[0].data(), packets_b[0].size()).valid, "second tile should begin as partial");
...
void test_bounds_active_reassembly_memory_and_evicts_oldest() {
    ...
    {
        const auto packets =
            make_packets(state, WD_TILE_16x16, tile_id, 40 + tile_id, make_tile_bytes(WD_TILE_16x16, 120 + tile_id), false, false);
...
void test_decompression_failure_immediately_queues_repair() {
    ...
    require(wd_udp_tile_packet_encode_header(packet.data(), packet.size(), &header), "invalid compressed test header");

    TileReassembler reassembler;
    require(!reassembler.process_udp_packet(state, packet.data(), packet.size()).valid, "corrupt compressed payload should not complete");
...
void test_conflicting_duplicate_fragment_poisoning_is_rejected() {
    ClientState state;
    initialize_state(state, 64, 64, 400);
    const auto packets = make_packets(state, WD_TILE_16x16, 0, 60, make_tile_bytes(WD_TILE_16x16, 130), false, false);
    require(packets.size() > 1, "conflict test requires a fragmented tile");

    TileReassembler reassembler;
    require(!reassembler.process_udp_packet(state, packets[0].data(), packets[0].size()).valid, "first fragment should remain partial");
...
void test_conflicting_duplicate_fragment_zero_metadata_is_rejected() {
    ClientState state;
    initialize_state(state, 64, 64, 400);
    const auto packets = make_packets(state, WD_TILE_16x16, 0, 61, make_tile_bytes(WD_TILE_16x16, 131), false, true);
    ...
    require(state.stats.tile_fragment_conflicts.load() == 1, "fragment-zero metadata conflict should be counted");
    require(reassembler.active_entry_count() == 0, "metadata-conflicted partial tile should be discarded");
...
void test_packet_validation_reports_rejection_reason() {
    ClientState state;
    initialize_state(state, 64, 64, 400);
    const auto                 packets = make_packets(state, WD_TILE_16x16, 0, 70, make_tile_bytes(WD_TILE_16x16, 140), false, false);
    wd_udp_tile_packet_decoded header{};
    require(wd_udp_tile_packet_decode(packets[0].data(), packets[0].size(), &header), "validation reason header decode");
