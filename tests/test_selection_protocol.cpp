...
void test_payload_roundtrip() {
    ...
    CHECK(wd_selection_payload_encode(session, token, WD_SELECTION_MIME_TEXT_UTF8, text, sizeof(text), payload.data(), payload.size(),
                                      &payload_size));
...
void test_encode_rejections() {
    ...
    CHECK(!wd_selection_payload_encode(0, 1, WD_SELECTION_MIME_TEXT_UTF8, nullptr, 0, payload, sizeof(payload), &size));
    CHECK(size == 0);
    CHECK(!wd_selection_payload_encode(1, 0, WD_SELECTION_MIME_TEXT_UTF8, nullptr, 0, payload, sizeof(payload), &size));
    CHECK(!wd_selection_payload_encode(1, 1, 99, nullptr, 0, payload, sizeof(payload), &size));
    CHECK(!wd_selection_payload_encode(1, 1, WD_SELECTION_MIME_TEXT_UTF8, embedded_nul, sizeof(embedded_nul), payload, sizeof(payload),
                                       &size));
    CHECK(!wd_selection_payload_encode(1, 1, WD_SELECTION_MIME_TEXT_UTF8, nullptr, 0, payload, sizeof(wd_selection_payload_header) - 1,
                                       &size));
    CHECK(wd_selection_payload_encode(1, 1, WD_SELECTION_MIME_TEXT_UTF8, nullptr, 0, payload, sizeof(payload), &size));
