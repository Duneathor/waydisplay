...
void stress_resize_keeps_transport_identity() {
    ...
    for (uint64_t iteration = 0; iteration < 20000; ++iteration)
    {
        ...
        require(client_normalize_and_validate_server_config(next, &error),
                "stress resize should remain a valid negotiated geometry");
        const uint32_t changes = client_classify_server_config_change(current, next);
        require((changes & ClientConfigChangeTransport) == 0,
                "ordinary resize must never recreate the transport");
        require((changes & ClientConfigChangeEpoch) != 0 &&
                    (changes & ClientConfigChangeContent) != 0,
...
void stress_video_transition_invariants() {
    ...
    for (uint64_t iteration = 0; iteration < 100000; ++iteration)
    {
        ...
        const ClientVideoTransitionDecision decision = client_video_transition(
            phase, advanced, eos, resize, keyframe, payload);

        require(!decision.accept_payload || payload,
                "transition cannot accept an empty video payload");
        ...
        if (resize)
        {
            require(decision.next_phase == ClientVideoPhase::AwaitingKeyframe,
                    "resize must leave the decoder waiting for a keyframe");
        }
        else if (eos)
        {
            require(decision.next_phase == ClientVideoPhase::Tiles,
                    "EOS must return to tile ownership");
        }
        ...
        else if (phase == ClientVideoPhase::Tiles)
        {
            ...
            require(ownership.is_current(epoch, ClientContentOwner::Tiles),
...
void stress_server_keyframe_epoch_commit() {
    ...
    for (uint64_t iteration = 0; iteration < 100000; ++iteration)
    {
        ...
        require(plan.source_content_epoch == epoch && plan.commit_on_queue,
                "entry plan should reserve from the current epoch");
        require(!wd_video_entry_plan_can_commit(&plan, epoch, false),
                "failed queue cannot commit a reserved epoch");
        require(wd_video_entry_plan_can_commit(&plan, epoch, true),
                "successful queue should commit the reserved keyframe epoch");
