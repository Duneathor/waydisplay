...
void test_rejects_invalid_and_mismatched_frames() {
    ...
    require(!queue.push_decoded(std::move(invalid), 4, 4, 1, 10, 1), "invalid decoded buffers must be rejected");
...
void test_full_queue_discards_tail_but_preserves_head() {
    ...
    require(queue.push_decoded(std::move(reuse), 4, 4, 12, 300, 4), "new decode should replace the discarded tail");
    require(queue.size() == 2, "queue should return to capacity");

    ClientQueuedVideoFrame first = queue.pop_front();
    require(first.frame_id == 10 && first.pts_usec == 100 && first.epoch == 3, "head metadata should survive overflow");
    ClientQueuedVideoFrame second = queue.pop_front();
    require(second.frame_id == 12 && second.pts_usec == 300 && second.epoch == 4, "replacement frame should follow the held head");
}
...
void test_recycle_and_clear_release_all_state() {
    ...
    require(!dropped_newest && decode.valid() && decode.width == 6, "explicit recycle buffer should be returned exactly once");
