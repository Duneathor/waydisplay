...
int main() {
    ...
    require(wd_audio_pcm_ring_read(&ring, output, 4, &pts, &continuous) == 4,
            "read exact first block");
    ...
    require(wd_audio_pcm_ring_write_planar(&ring, planes, 3, 300),
            "write PipeWire planar stereo block");
    require(wd_audio_pcm_ring_read(&ring, output, 3, &pts, &continuous) == 3,
            "read planar block as interleaved PCM");
    require(pts == 300 && continuous, "planar block PTS continuity");
    require(output[0] == left[0] && output[1] == right[0] &&
                output[4] == left[2] && output[5] == right[2],
    ...
    require(wd_audio_packetizer_make_packet(&packetizer, 0, 960, 20, &header),
            "packetize first audio frame");
    ...
    require(wd_audio_packetizer_make_packet(&packetizer, 960, 960, 20, &header),
            "packetize continuous audio frame");
    require(header.sequence == 2 && header.flags == 0,
            "continuous audio should not carry a discontinuity flag");
    require(wd_audio_packetizer_make_packet(&packetizer, 3000, 960, 20, &header),
            "packetize timestamp gap");
    require((header.flags & WD_AUDIO_PACKET_DISCONTINUITY) != 0,
            "timestamp gaps should be explicit on the wire");
    require(wd_audio_packetizer_make_eos(&packetizer, 3960, &header),
            "packetize audio EOS");
    require(header.duration_samples == 0 && header.data_size == 0 &&
                header.flags == WD_AUDIO_PACKET_END_OF_STREAM,
            "audio EOS should be canonical");
    require(wd_audio_packet_payload_size_is_valid(&header, sizeof(header)),
            "canonical audio EOS should validate");
    header.flags |= WD_AUDIO_PACKET_DISCONTINUITY;
    require(!wd_audio_packet_payload_size_is_valid(&header, sizeof(header)),
            "audio EOS must reject ambiguous flag combinations");
    return 0;
}
