...
void test_stable_geometry_reconnect_accepts_cached_tiles() {
    ...
    require(configure_client_dirty_tile_grid(grid, 1422, 773, 16, 16),
            "initial reconnect configuration should allocate the dirty grid");
    require(grid.mark_rect({1408, 768, 14, 5}),
            "the final partial tile from a cached refresh should be accepted");
    require(grid.dirty_tile_count() == 1,
            "a cached edge tile should produce one pending base tile");

    std::vector<ClientDirtyRect> rects;
    grid.take_rects(rects);
    require(rects.size() == 1, "the cached edge tile should be renderable");
    require(rects.front().x == 1408 && rects.front().y == 768 &&
                rects.front().w == 14 && rects.front().h == 5,
...
void test_audio_held_video_survives_decode_and_recovery_cycle() {
    ...
    require(queue.push_decoded(make_frame(1), 4, 4, 1, 1050000, 9),
            "first decoded frame should enter the presentation queue");
    require(queue.push_decoded(make_frame(2), 4, 4, 2, 1070000, 9),
            "second decoded frame should enter the presentation queue");
    require(queue.push_decoded(make_frame(3), 4, 4, 3, 1090000, 9),
            "third decoded frame should enter the presentation queue");
    ...
    const ClientVideoAudioSyncPlan held =
        client_video_audio_sync_plan(head->pts_usec, 48000, 48000);
    require(held.decision == ClientVideoAudioSyncDecision::Hold,
            "a frame 50 ms ahead of audio should wait");
    require(held.retry_after_ms == 10,
            "the renderer should wake when the frame crosses the 40 ms early threshold");
    ...
    require(queue.front() && queue.front()->frame_id == 1,
            "continued decoding must not overwrite the audio-held head");
    ...
    require(wd_client_video_health_classify(&health) ==
                WD_CLIENT_VIDEO_HEALTH_AUDIO_WAIT,
            "queued frames waiting for audio must not be classified as decoder failure");

    require(wd_tile_recovery_decide(false, 0, 1, 5) == WD_TILE_RECOVERY_WAIT,
            "tile recovery should wait until its refresh has drained");
    require(!wd_video_entry_allowed(true, 0),
            "forced video entry must remain blocked during tile recovery");
    ...
    require(wd_tile_recovery_decide(true, 1, 4, 5) ==
    ...
    require(!wd_video_entry_allowed(false, 1),
            "video should remain blocked during its post-recovery cooldown");
    require(wd_video_entry_allowed(false, 0),
            "video may resume after recovery and cooldown complete");

    const ClientVideoAudioSyncPlan due =
        client_video_audio_sync_plan(queue.front()->pts_usec, 48480, 48000);
    ...
    require(presented.frame_id == 1,
            "the frame that waited for audio should be presented rather than overwritten");
}
...
void test_bootstrap_refresh_does_not_trigger_video_selection() {
    ...
    require(wd_video_auto_entry_evaluate(&metrics).candidate,
            "the same sustained scene cost should normally select video mode");
    metrics.selection_suppressed = true;
    require(!wd_video_auto_entry_evaluate(&metrics).candidate,
            "cached reconnect and recovery refresh traffic must not select video mode");
}
...
void test_reconnect_telemetry_survives_protocol_copy() {
    ...
    require(received.video_frames_decoded == sent.video_frames_decoded &&
                received.video_audio_sync_holds == sent.video_audio_sync_holds &&
                received.video_queue_depth == sent.video_queue_depth &&
                received.video_queue_depth_max == sent.video_queue_depth_max &&
