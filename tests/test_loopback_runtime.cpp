...
std::vector<std::vector<uint8_t>> make_packets(const ClientState& state, uint16_t tile_id, uint64_t generation,
...
void test_udp_loopback_reorders_and_deduplicates_fragments() {
    ...
    for (size_t index : order)
    {
        ...
        require(sendto(sender, packet.data(), packet.size(), 0, reinterpret_cast<const sockaddr*>(&address), sizeof(address)) ==
