...
void test_rejects_oversized_framebuffer() {
    ...
    ClientConfigValidationError error{};
    require(!client_normalize_and_validate_server_config(config, &error), "render width above the protocol limit should fail");
    require(error == ClientConfigValidationError::UnsupportedDisplayDimensions, "oversized geometry should report the dimension limit");

    config        = valid_config();
    config.height = WD_MAX_RENDER_HEIGHT + 1u;
    require(!client_normalize_and_validate_server_config(config, &error), "render height above the protocol limit should fail");
}
...
void test_classifies_transport_changes_independently_of_geometry() {
    ...
    require((flags & ClientConfigChangeTransport) == 0, "resize with stable session/token must retain UDP transport");
...
void test_rejects_missing_config_epoch_and_invalid_capabilities() {
    ...
    ClientConfigValidationError error{};
    require(!client_normalize_and_validate_server_config(config, &error), "zero config epoch should fail");
    ...
    require(!client_normalize_and_validate_server_config(config, &error), "unknown server capabilities should fail");
    require(error == ClientConfigValidationError::InvalidCapabilities, "unknown server capabilities should report a capability failure");

    config                 = valid_config();
    config.capabilities    = WD_SERVER_CAP_VIDEO_STREAM;
    config.video_codecs    = 0;
    config.video_transport = WD_VIDEO_TRANSPORT_TCP;
    require(!client_normalize_and_validate_server_config(config, &error), "video capability without a codec should fail");
}
