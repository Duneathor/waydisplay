...
std::vector<std::vector<uint8_t>> make_uncompressed_packets(const ClientState& state, uint16_t tile_id, uint64_t generation) {
    ...
    for (uint8_t packet_id = 0; packet_id < count; ++packet_id)
    {
        ...
        require(wd_udp_tile_packet_encode_header(packet.data(), packet.size(), &header), "edge-test packet header should encode");
...
void test_short_header_identity_geometry_and_fragment_counters() {
    ...
    require(!reassembler.process_udp_packet(state, nullptr, 0).valid, "null datagram should be rejected");
    ...
    require(!reassembler.process_udp_packet(state, malformed.data(), malformed.size()).valid, "malformed header should be rejected");
    require(state.stats.udp_invalid_header.load() == 1, "header decoder rejection should have a distinct counter");

    auto                       packets = make_uncompressed_packets(state, 0, 1);
    wd_udp_tile_packet_decoded header{};
    require(wd_udp_tile_packet_decode(packets[0].data(), packets[0].size(), &header), "canonical edge-test header should decode");
...
void test_payload_larger_than_budget_drops_without_entry() {
    ...
    require(state.stats.reassembly_budget_drops.load() == 1, "oversized tile should increment the budget-drop counter");
...
void test_partial_timeout_reduces_adaptive_deadline_and_queues_repair() {
    ...
    require(state.stats.partial_tiles_timed_out.load() == 1 && state.stats.partial_tile_missing_packets.load() == 2,
            "timeout should count the tile and each missing fragment");
    require(!state.retx_queue.empty() && state.stats.partial_tile_retx_queued.load() == 1, "timeout should queue immediate repair work");
    ...
    require(reassembler.next_expiry_deadline_ns(state) == 0, "empty reassembler should have no expiry deadline");
}
...
void test_reset_and_completed_buffer_pool_bounds() {
    ...
    TileReassembler reassembler;
    require(!reassembler.process_udp_packet(state, packets[0].data(), packets[0].size()).valid, "reset test should create a partial entry");
    require(reassembler.active_entry_count() == 1 && reassembler.slot_count() != 0, "partial entry should configure slot tables");
    reassembler.reset();
    require(reassembler.active_entry_count() == 0 && reassembler.active_payload_bytes() == 0 && reassembler.slot_count() == 0,
            "reset should clear active entries, payload accounting, and slot geometry");

    std::vector<uint8_t> oversized;
    oversized.reserve(static_cast<size_t>(WD_WIRE_TILE_MAX_WIDTH) * WD_WIRE_TILE_MAX_HEIGHT * WD_BYTES_PER_PIXEL + 1u);
    reassembler.recycle_completed_tile_buffer(std::move(oversized));
    require(reassembler.recycled_completed_buffer_count() == 0, "oversized completed buffers should not be retained");
    ...
    require(reassembler.recycled_completed_buffer_count() == 8, "completed buffer recycle pool should remain bounded");
}
