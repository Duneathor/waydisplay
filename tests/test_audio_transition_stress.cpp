...
void stress_audio_negotiation_and_opt_out() {
    for (uint32_t iteration = 0; iteration < 100000; ++iteration)
    {
        ...
        require(wd_client_hello_payload_is_valid(&hello, sizeof(hello)),
                "enabled and opt-out audio hellos must both be canonical");
        if (!enabled)
        {
            require((hello.capabilities & WD_CLIENT_CAP_AUDIO_STREAM) == 0 &&
                        hello.audio_codecs == 0 && hello.audio_transport == 0 &&
        ...
        if (!enabled)
        {
            require(!wd_client_hello_payload_is_valid(&malformed, sizeof(malformed)),
                    "disabled audio must reject residual codec fields");
        }
    }
}
...
void stress_packet_epochs_and_discontinuities() {
    ...
    for (uint32_t iteration = 0; iteration < 100000; ++iteration)
    {
        ...
        require(wd_audio_packetizer_make_packet(&packetizer, pts,
                                                 WD_AUDIO_FRAME_SAMPLES_DEFAULT,
                                                 data_size, &header),
                "valid Opus packets must be packetized");
        require(header.session_id == session && header.connection_token == token &&
                    header.audio_epoch == epoch && header.media_clock_id == clock,
                "packet identity must follow the current connection and audio epoch");
        require(header.sequence == previous_sequence + 1,
                "audio packet sequence must be strictly increasing within an epoch");
        require(wd_audio_packet_payload_size_is_valid(
                    &header, static_cast<uint32_t>(sizeof(header)) + header.data_size),
                "packetizer output must pass wire validation");
        if (previous_sequence == 0 || explicit_gap || forced)
        {
            require((header.flags & WD_AUDIO_PACKET_DISCONTINUITY) != 0,
                    "new epochs and timeline gaps must be explicit");
        }
...
void stress_ring_wrap_and_overrun() {
    ...
    for (uint32_t iteration = 0; iteration < 50000; ++iteration)
    {
        ...
        if ((iteration % 3u) != 0)
        {
            ...
            {
                require(read_pts == expected_read_pts && continuous,
                        "ring wrap must preserve sample timestamps");
    ...
    require(successful_writes != 0 && successful_reads != 0,
            "stress ring must exercise both producer and consumer");
    require(wd_audio_pcm_ring_overruns(&ring) != 0,
            "bounded ring must report sustained producer pressure");
    require(wd_audio_pcm_ring_queued_frames(&ring) <= ring.capacity_frames,
            "ring occupancy must remain bounded");
...
void stress_media_clock_sync_and_budget() {
    ...
    for (uint64_t usec = 0; usec < 10000000ull; usec += 137ull)
    {
        ...
        const ClientVideoAudioSyncDecision decision =
            client_video_audio_sync_decide(usec, samples, WD_AUDIO_SAMPLE_RATE_DEFAULT);
        require(decision == ClientVideoAudioSyncDecision::Present,
                "matching audio/video timestamps should present immediately");
    }
    ...
    for (uint32_t iteration = 0; iteration < 100000; ++iteration)
    {
        ...
        require(tile_budget != 0 && tile_budget <= measured,
                "audio reservation must preserve a bounded nonzero tile budget");
    }
}
