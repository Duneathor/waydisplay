...
std::vector<uint8_t> encode(const uint8_t* text, uint32_t size,
                            uint8_t session_id = SESSION_ID,
                            uint64_t token = TOKEN) {
    std::vector<uint8_t> payload(sizeof(wd_selection_payload_header) + size);
    uint32_t payload_size = 0;
    CHECK(wd_selection_payload_encode(session_id, token,
                                      WD_SELECTION_MIME_TEXT_UTF8,
                                      text, size, payload.data(), payload.size(),
                                      &payload_size));
    payload.resize(payload_size);
    return payload;
}

wd_selection_text_view decode(const std::vector<uint8_t>& payload,
                              uint8_t session_id = SESSION_ID,
                              uint64_t token = TOKEN) {
    wd_selection_text_view view{};
    CHECK(wd_selection_payload_decode(payload.data(),
                                      static_cast<uint32_t>(payload.size()),
                                      session_id, token, &view));
    return view;
}
...
void test_server_to_client_and_echo_suppression() {
    ...
    CHECK(waydisplay::client_selection_sync_should_apply(
        client, waydisplay::ClientSelectionKind::Clipboard, received));
    waydisplay::client_selection_sync_note_applied(
        client, waydisplay::ClientSelectionKind::Clipboard, received);

    CHECK(!waydisplay::client_selection_sync_should_send(
        client, waydisplay::ClientSelectionKind::Clipboard, received, false));
    CHECK(waydisplay::client_selection_sync_should_send(
        client, waydisplay::ClientSelectionKind::Clipboard, received, true));
...
void test_client_to_server_and_reconnect() {
    ...
    CHECK(waydisplay::client_selection_sync_should_send(
        client, waydisplay::ClientSelectionKind::Clipboard, value, false));
    waydisplay::client_selection_sync_note_sent(
        client, waydisplay::ClientSelectionKind::Clipboard, value);
    CHECK(!waydisplay::client_selection_sync_should_send(
        client, waydisplay::ClientSelectionKind::Clipboard, value, false));

    const auto payload = encode(reinterpret_cast<const uint8_t*>(value.data()),
