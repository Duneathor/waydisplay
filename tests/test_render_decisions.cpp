...
void test_dirty_grid_invalid_and_empty_paths() {
    ...
    require(grid.dirty_tile_count() == 0 && !grid.mark_rect({0, 0, 1, 1}), "invalid reset should clear previous dirty work");
...
void test_coalescing_and_bounds_edge_paths() {
    ...
    require(overlap.size() == 1 && overlap[0].x == 0 && overlap[0].y == 0 && overlap[0].w == 16 && overlap[0].h == 16,
    ...
    require(bounds.x == 1 && bounds.y == 2 && bounds.w == 9 && bounds.h == 8, "bounds should cover the union of all rectangles");
}

void test_upload_plan_branch_matrix() {
    require(plan_dirty_texture_upload({}, 100, 100).mode == DirtyTextureUploadMode::Full, "empty dirty work should request a full upload");
    ...
    const DirtyTextureUploadPlan bounds_plan = plan_dirty_texture_upload({{10, 10, 10, 10}, {30, 10, 10, 10}}, 1000, 1000, costs);
    require(bounds_plan.mode == DirtyTextureUploadMode::Bounds && bounds_plan.bounds.x == 10 && bounds_plan.bounds.w == 30,
...
void test_cost_observation_limits_and_sample_paths() {
    ...
    require(model.update_samples == 1 && model.pixel_samples == 0, "small updates should calibrate fixed cost only");
    observe_texture_upload_call(model, true, 200000, 65536);
    require(model.lock_samples == 1 && model.pixel_samples == 1, "large lock uploads should calibrate fixed and variable cost");
    ...
    require(model.update_samples == UINT32_MAX && model.pixel_samples == UINT32_MAX && model.snapshot_samples == UINT32_MAX,
...
void test_summary_index_consistency_checks() {
    ...
    require(summary_pending_index_add(active, positions, 1) && active.size() == 2, "duplicate add should be idempotent");
    require(summary_pending_index_remove(active, positions, 1), "indexed tile should be removable");
    require(active.size() == 1 && active[0] == 3 && positions[3] == 0, "removal should repair the moved tile position");
...
void test_present_telemetry_filters_and_time_safety() {
    ...
    require(batch.completion_age_sum_ns == 0, "future completion timestamps must not underflow age accumulation");
    require(batch.input_sequence_count == 1 && batch.input_sequences[0] == 12, "duplicate and zero input sequences should be ignored");

    commit_tile_present_telemetry(pending, updates, 7);
    require(pending[1].completion_id == 0 && pending[2].completion_id == 0 && pending[3].completion_id == 6,
