...
uint32_t wd_fnv1a_tile_hash_xrgb8888_for(const uint32_t* framebuffer_xrgb8888, uint32_t framebuffer_width, uint32_t framebuffer_height,
    ...
    const uint32_t start_x        = wd_tile_start_x_for(tile_id, tiles_x);
    const uint32_t start_y        = wd_tile_start_y_for(tile_id, tiles_x);
    const uint32_t visible_width  = wd_tile_visible_width_for(framebuffer_width, tile_id, tiles_x);
    const uint32_t visible_height = wd_tile_visible_height_for(framebuffer_height, tile_id, tiles_x);
...
bool wd_extract_tile_xrgb8888_for(const uint32_t* framebuffer_xrgb8888, uint32_t framebuffer_width, uint32_t framebuffer_height,
    ...
    if (!framebuffer_xrgb8888 || !out_tile_bytes || !wd_tile_id_valid_for(tile_id, total_tiles))
    {
        return false;
    }
...
bool wd_blit_tile_xrgb8888_for(uint32_t* framebuffer_xrgb8888, uint32_t framebuffer_width, uint32_t framebuffer_height, uint16_t tiles_x,
    ...
    if (!framebuffer_xrgb8888 || !tile_bytes || !wd_tile_id_valid_for(tile_id, total_tiles))
    {
        return false;
    }
