...
enum wd_tile_recovery_action wd_tile_recovery_decide(bool refresh_sent,
...
wd_client_video_health_classify(const struct wd_client_video_health_metrics* metrics) {
    ...
    if (metrics->client_frames_decoded != 0 && metrics->client_audio_sync_holds != 0 &&
