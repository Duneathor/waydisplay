...
static void schedule_decoration_configure(struct wd_xdg_decoration *decoration) {
    if (!decoration || !decoration->decoration ||
        decoration->configure_idle) {
        return;
    }

    struct wl_display *display =
        wl_client_get_display(decoration->decoration->resource->client);
    struct wl_event_loop *event_loop = wl_display_get_event_loop(display);

    decoration->configure_idle =
        wl_event_loop_add_idle(event_loop,
                               xdg_decoration_configure_idle,
                               decoration);
}

static void handle_xdg_decoration_request_mode(struct wl_listener *listener,
                                               void *data) {
    (void)data;

    struct wd_xdg_decoration *decoration =
        wl_container_of(listener, decoration, request_mode);

    schedule_decoration_configure(decoration);
}

static void handle_xdg_decoration_destroy(struct wl_listener *listener,
                                          void *data) {
    (void)data;
...
static void handle_new_xdg_toplevel_decoration(struct wl_listener *listener,
                                               void *data) {
    struct wd_server *server =
        wl_container_of(listener, server, new_xdg_toplevel_decoration);
    ...
    if (!decoration) {
        wlr_log(WLR_ERROR,
                "WayDisplay: failed to allocate xdg-decoration state");
        return;
    }
    ...
    (void)server;
    wlr_log(WLR_INFO, "WayDisplay: queued xdg-decoration client-side mode");
}
...
bool wd_xdg_decoration_init(struct wd_server *server) {
    ...
    if (!server->xdg_decoration_manager) {
        wlr_log(WLR_ERROR,
                "WayDisplay: failed to create xdg-decoration manager");
        return false;
    }

    server->new_xdg_toplevel_decoration.notify =
        handle_new_xdg_toplevel_decoration;
    wl_signal_add(
