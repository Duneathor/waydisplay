...
uint32_t wd_tile_wire_bytes_for_payload(uint32_t payload_size, uint16_t udp_payload_target, uint16_t packet_header_size,
    ...
    const uint64_t wire_bytes = (uint64_t)payload_size + first_packet_header_size + (uint64_t)(packet_count - 1u) * packet_header_size;
...
static uint16_t wd_percent_clamped(uint64_t numerator, uint64_t denominator) {
    ...
    const uint64_t percent = numerator > UINT64_MAX / 100u ? UINT64_MAX : (numerator * 100u) / denominator;
...
bool wd_tile_compression_is_worthwhile(uint32_t compressed_size, uint32_t uncompressed_size, uint16_t udp_payload_target,
    ...
    const uint32_t compressed_wire =
        wd_tile_wire_bytes_for_payload(compressed_size, udp_payload_target, packet_header_size, first_packet_header_size);
    const uint32_t uncompressed_wire =
        wd_tile_wire_bytes_for_payload(uncompressed_size, udp_payload_target, packet_header_size, first_packet_header_size);
    ...
    if (minimum_savings_percent != 0 && (uint64_t)saved * 100u < (uint64_t)uncompressed_wire * minimum_savings_percent)
    {
        return false;
    }
    return true;
...
bool wd_tile_xrgb_payload_may_compress(const uint8_t* payload, uint32_t payload_size) {
    ...
    {
        const uint32_t pixel_index = samples == 1 ? 0 : (uint32_t)(((uint64_t)i * (pixel_count - 1u)) / (samples - 1u));
        uint32_t       color       = 0;
    ...
    return unique_count <= samples * WD_TILE_ADVISOR_MAX_UNIQUE_NUMERATOR / WD_TILE_ADVISOR_MAX_UNIQUE_DENOMINATOR ||
           adjacent_repeats >= WD_TILE_ADVISOR_MIN_ADJACENT_REPEATS || repeated_deltas >= samples / WD_TILE_ADVISOR_REPEATED_DELTA_DIVISOR;
}
...
size_t wd_tile_select_local_region_index(const uint16_t* region_ids, size_t region_count, uint16_t regions_x, uint16_t cursor_region_id,
                                         const uint64_t* region_enqueued_ns, size_t region_capacity, uint64_t now_ns,
    ...
    for (size_t i = 1; i < region_count; ++i)
    {
        ...
        if (distance == best_distance)
        {
            ...
            const uint64_t queued      = region_enqueued_ns && region_id < region_capacity ? region_enqueued_ns[region_id] : 0;
            if ((queued != 0 && (best_queued == 0 || queued < best_queued)) || (queued == best_queued && region_id < best_id))
            {
                best_index = i;
            }
        }
