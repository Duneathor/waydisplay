...
static void xdg_dialog_handle_destroy(struct wl_client* client, struct wl_resource* resource) {
    (void)client;
    wl_resource_destroy(resource);
}

static void xdg_dialog_handle_set_modal(struct wl_client* client, struct wl_resource* resource) {
    (void)client;
...
static void xdg_dialog_handle_unset_modal(struct wl_client* client, struct wl_resource* resource) {
    (void)client;
...
static void xdg_wm_dialog_handle_destroy(struct wl_client* client, struct wl_resource* resource) {
    (void)client;
    wl_resource_destroy(resource);
}

static void xdg_wm_dialog_handle_get_xdg_dialog(struct wl_client* client, struct wl_resource* resource, uint32_t id,
    ...
    struct wd_view* view = wd_scene_view_from_xdg_toplevel_resource(server, toplevel_resource);

    if (view && view->xdg_dialog_resource)
    {
        wl_resource_post_error(resource, XDG_WM_DIALOG_V1_ERROR_ALREADY_USED, "xdg_toplevel already has an xdg_dialog_v1");
        return;
    }

    struct wl_resource* dialog_resource = wl_resource_create(client, &xdg_dialog_v1_interface, wl_resource_get_version(resource), id);
    if (!dialog_resource)
    {
        wl_resource_post_no_memory(resource);
        return;
     ...
     */
    wl_resource_set_implementation(dialog_resource, &xdg_dialog_impl, view, xdg_dialog_resource_destroy);
...
static void bind_xdg_dialog_manager(struct wl_client* client, void* data, uint32_t version, uint32_t id) {
    struct wd_server* server = data;

    struct wl_resource* resource = wl_resource_create(client, &xdg_wm_dialog_v1_interface, version, id);
    if (!resource)
    {
    ...
    wl_resource_set_implementation(resource, &xdg_wm_dialog_impl, server, NULL);
}
