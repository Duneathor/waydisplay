...
void wd_scene_note_dialog_state(struct wd_view* view) {
    ...
    WD_LOG_DEBUG("xdg-dialog view=%p app_id=%s title=%s modal=%d parent=%p", (void*)view, view_app_id(view), view_title(view),
...
static void track_surface_commit_iterator(struct wlr_surface* surface, int sx, int sy, void* data) {
    ...
    if (!state)
    {
        WD_LOG_ERROR("failed to allocate xdg surface commit tracker");
        return;
    }
...
static void view_configure_idle(void* data) {
    ...
    WD_LOG_DEBUG("sent initial xdg toplevel configure");
}
...
static void view_handle_map(struct wl_listener* listener, void* data) {
    ...
    WD_LOG_DEBUG("xdg toplevel mapped scene_tree=%p", (void*)view->scene_tree);
}
...
static void view_schedule_popup_unconstrain(struct wd_view* view, struct wlr_xdg_popup* popup) {
    ...
    if (!state)
    {
        WD_LOG_ERROR("failed to allocate popup unconstrain state");
        return;
    }
...
static bool popup_commit_tracker_ensure_scene_tree(struct wd_popup_commit_tracker* state) {
    ...
    if (!parent_tree)
    {
        WD_LOG_DEBUG("not creating explicit popup scene tree popup=%p view=%p because "
...
static void popup_commit_tracker_mark_dirty(struct wd_popup_commit_tracker* state, const char* reason) {
    ...
    if (geometry_changed)
    {
        WD_LOG_DEBUG("xdg popup %s popup=%p base=%p parent=%p "
...
static void view_track_popup_commits(struct wd_view* view, struct wlr_xdg_popup* popup) {
    ...
    {
        WD_LOG_DEBUG("xdg popup already tracked popup=%p view=%p", (void*)popup, (void*)view);
        return;
    }

    if (!view)
    {
        WD_LOG_DEBUG("not tracking xdg popup because view is NULL popup=%p", (void*)popup);
        return;
    }

    if (!popup)
    {
        WD_LOG_DEBUG("not tracking xdg popup for view=%p because popup is NULL", (void*)view);
        return;
    }

    if (!popup->base)
    {
        WD_LOG_DEBUG("not tracking xdg popup popup=%p for view=%p because base is NULL", (void*)popup, (void*)view);
        return;
    }

    if (!popup->base->surface)
    {
        WD_LOG_DEBUG("not tracking xdg popup popup=%p base=%p for view=%p because base "
                     "surface is NULL",
    ...
    if (!state)
    {
        WD_LOG_ERROR("failed to allocate popup commit tracker");
        return;
    }
...
static void view_handle_set_app_id(struct wl_listener* listener, void* data) {
    ...
    WD_LOG_DEBUG("toplevel metadata app_id=%s title=%s", view_app_id(view), view_title(view));
}
...
static void view_handle_request_move(struct wl_listener* listener, void* data) {
    ...
    if (view->fullscreen)
    {
        WD_LOG_DEBUG("ignoring xdg_toplevel.request_move while fullscreen");
        return;
    }
...
static void view_handle_request_resize(struct wl_listener* listener, void* data) {
    ...
    if (view->fullscreen)
    {
        WD_LOG_DEBUG("ignoring xdg_toplevel.request_resize while fullscreen");
        return;
    }
...
static void server_handle_new_xdg_toplevel(struct wl_listener* listener, void* data) {
    ...
    WD_LOG_DEBUG("new xdg toplevel scene_tree=%p", (void*)view->scene_tree);
