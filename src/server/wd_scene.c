...
struct wd_view *wd_scene_view_at(struct wd_server *server,
    ...
    /*
     * Walk newest/topmost first. Since wd_scene_raise_view() moves scene nodes,
     * this list is only approximate, but good enough for the current simple
     * compositor. Later we can use wlr_scene_node_at().
     */
