...
bool wd_audio_pcm_ring_write(struct wd_audio_pcm_ring* ring, const float* samples,
    ...
    for (uint32_t frame = 0; frame < frames; ++frame)
    {
        ...
        memcpy(&ring->samples[(size_t)slot * ring->channels],
               &samples[(size_t)frame * ring->channels],
...
bool wd_audio_pcm_ring_write_planar(struct wd_audio_pcm_ring* ring,
                                    ...
                                    uint32_t frames, uint64_t first_pts_sample) {
    if (!ring || !ring->samples || !ring->pts_samples || !planes || frames == 0 ||
        frames > ring->capacity_frames)
    {
        return false;
    }
    ...
    const uint64_t write = __atomic_load_n(&ring->write_frame, __ATOMIC_RELAXED);
    const uint64_t read = __atomic_load_n(&ring->read_frame, __ATOMIC_ACQUIRE);
    if (write - read > ring->capacity_frames ||
        frames > ring->capacity_frames - (uint32_t)(write - read))
    {
        __atomic_fetch_add(&ring->overruns, 1, __ATOMIC_RELAXED);
        return false;
    }
...
uint32_t wd_audio_pcm_ring_read(struct wd_audio_pcm_ring* ring, float* samples,
    ...
    for (uint32_t frame = 0; frame < frames; ++frame)
    {
        ...
        previous = pts;
        memcpy(&samples[(size_t)frame * ring->channels],
               &ring->samples[(size_t)slot * ring->channels],
               (size_t)ring->channels * sizeof(float));
    }
