...
static bool add_text_mime_types(struct wl_array *mime_types) {
  return add_mime_type(mime_types, "text/plain;charset=utf-8") &&
         add_mime_type(mime_types, "text/plain") &&
         add_mime_type(mime_types, "UTF8_STRING") &&
         add_mime_type(mime_types, "TEXT") &&
         add_mime_type(mime_types, "STRING");
}
...
static void remote_data_source_send(struct wlr_data_source *source,
                                    const char *mime_type,
                                    int fd) {
  struct wd_remote_data_source *remote =
      wl_container_of(source, remote, source);
...
static void remote_primary_source_send(
    struct wlr_primary_selection_source *source,
    const char *mime_type,
    int fd) {
  struct wd_remote_primary_source *remote =
      wl_container_of(source, remote, source);
  struct wd_server *server = remote->server;
...
static void remote_primary_source_destroy(
    struct wlr_primary_selection_source *source) {
  struct wd_remote_primary_source *remote =
      wl_container_of(source, remote, source);
...
static void clear_remote_selection_source(struct wd_server *server,
                                          bool primary);
...
static bool set_source_text_copy(uint8_t **dst_text,
                                 uint32_t *dst_size,
...
static bool publish_remote_clipboard_selection(struct wd_server *server) {
  ...
  if (!set_source_text_copy(&source->text,
                            &source->text_size,
                            server->remote_clipboard_text,
                            server->remote_clipboard_text_size)) {
    free(source);
    return false;
  }
...
static bool publish_remote_primary_selection(struct wd_server *server) {
  ...
  if (!set_source_text_copy(&source->text,
                            &source->text_size,
                            server->remote_primary_text,
                            server->remote_primary_text_size)) {
    free(source);
    return false;
  }
...
static void update_keyboard_modifiers_for_key(struct wd_server *server,
...
static void synthesize_key(struct wd_server *server,
                           uint32_t evdev_key_code,
                           bool pressed,
                           uint32_t time_msec) {
...
static bool keyboard_modifier_active(struct wd_server *server,
                                     const char *name) {
  if (!server || !server->keyboard || !server->keyboard->xkb_state || !name) {
    return false;
  }

  return xkb_state_mod_name_is_active(server->keyboard->xkb_state,
                                      name,
                                      XKB_STATE_MODS_EFFECTIVE) > 0;
}

static void synthesize_clipboard_paste_shortcut(struct wd_server *server) {
  if (!server || !server->seat || !server->keyboard ||
  ...
  const bool ctrl_already_active =
      keyboard_modifier_active(server, XKB_MOD_NAME_CTRL);
  const bool shift_already_active =
      keyboard_modifier_active(server, XKB_MOD_NAME_SHIFT);
...
static bool remote_selection_matches(struct wlr_data_source *source,
                                     const uint8_t *text,
                                     uint32_t text_size) {
  if (!source) {
    return false;
  ...
  struct wd_remote_data_source *remote =
      wl_container_of(source, remote, source);
  return remote->text_size == text_size &&
         (text_size == 0 || memcmp(remote->text, text, text_size) == 0);
}

static void store_remote_selection(struct wd_server *server,
                                   uint8_t *text,
                                   uint32_t text_size,
                                   bool primary) {
  if (!server) {
  ...
  if (primary) {
    ...
    if (remote_selection_matches(server->remote_clipboard_source,
                                 text,
                                 text_size)) {
      free(server->remote_clipboard_text);
