...
static uint64_t wd_audio_graph_ticks_to_samples(uint64_t ticks, uint32_t rate_num, uint32_t rate_denom) {
    ...
    const __uint128_t scaled  = (__uint128_t)ticks * WD_AUDIO_SAMPLE_RATE_DEFAULT * rate_num;
...
static void wd_audio_stream_capture(void* userdata, const float* samples, uint32_t frames, uint8_t channels,
    ...
    const uint64_t observed_first_pts =
        wd_media_ns_to_samples(timing->cycle_start_ns, stream->media_clock_start_ns, WD_AUDIO_SAMPLE_RATE_DEFAULT);
    const uint64_t tolerance = (uint64_t)frames * WD_AUDIO_CAPTURE_TIMESTAMP_TOLERANCE_CYCLES;
    uint64_t       first_pts = observed_first_pts;

    if (timing->position_reliable)
    {
        bool clock_continuous = stream->capture_clock_valid && !timing->discontinuity && timing->clock_id == stream->capture_clock_id &&
        ...
        if (clock_continuous)
        {
            const uint64_t delta_ticks   = timing->position - stream->capture_clock_origin_position;
            const uint64_t delta_samples = wd_audio_graph_ticks_to_samples(delta_ticks, timing->rate_num, timing->rate_denom);
        ...
        else if (stream->capture_pts_valid && wd_audio_abs_difference(first_pts, stream->capture_next_pts) > tolerance)
        {
    ...
    else
    {
        ...
        if (stream->capture_pts_valid)
        {
            const uint64_t difference = wd_audio_abs_difference(observed_first_pts, stream->capture_next_pts);
...
static bool wd_audio_stream_send_config(struct wd_audio_stream* stream) {
    ...
    return wd_async_tcp_send_message(stream->tx, stream->tcp_fd, WD_MSG_AUDIO_CONFIG, &config, sizeof(config));
}

static void wd_audio_stream_drop_stale_queue(struct wd_audio_stream* stream) {
    uint32_t dropped = wd_async_tcp_sender_drop_message_type(stream->tx, WD_MSG_AUDIO_PACKET);
    if (dropped != 0)
    {
...
static void* wd_audio_stream_worker(void* userdata) {
    ...
    while (!__atomic_load_n(&stream->stop_requested, __ATOMIC_ACQUIRE))
    {
        ...
        const uint64_t captured_frames = __atomic_load_n(&stream->stats.captured_frames, __ATOMIC_RELAXED);
        if (captured_frames != 0 && !stream->capture_active_logged)
        {
            stream->capture_active_logged = true;
            WD_LOG_INFO("private audio route active: sink=%s captured_frames=%llu", wd_audio_capture_sink_name(stream->capture),
                        (unsigned long long)captured_frames);
        }
        else if (captured_frames == 0 && !stream->capture_idle_logged &&
                 wd_now_ns() - stream->capture_started_ns >= WD_AUDIO_CAPTURE_IDLE_DIAGNOSTIC_NS)
        {
            stream->capture_idle_logged = true;
            WD_LOG_DEBUG("private audio sink has received no PCM for %llu seconds: "
                         ...
                         wd_audio_capture_sink_name(stream->capture), wd_audio_capture_sink_target(stream->capture));
        }

        wd_async_tcp_sender_reap(stream->tx);

        if (wd_async_tcp_sender_pending_bytes(stream->tx) >= stream->tx_max_pending_bytes)
        {
        ...
        uint32_t got         = wd_audio_pcm_ring_read(&stream->ring, pcm, WD_AUDIO_FRAME_SAMPLES_DEFAULT, &pts_samples, &continuity);
        ...
        if (!continuity || __atomic_exchange_n(&stream->force_discontinuity, false, __ATOMIC_ACQ_REL))
        {
            if (!wd_audio_encoder_reset(stream->encoder))
            {
                __atomic_add_fetch(&stream->stats.encode_failures, 1, __ATOMIC_RELAXED);
                __atomic_store_n(&stream->force_discontinuity, true, __ATOMIC_RELEASE);
                continue;
            }
            wd_audio_packetizer_mark_discontinuity(&stream->packetizer);
            __atomic_add_fetch(&stream->stats.discontinuities, 1, __ATOMIC_RELAXED);
        }

        uint32_t packet_size = 0;
        if (!wd_audio_encoder_encode(stream->encoder, pcm, WD_AUDIO_FRAME_SAMPLES_DEFAULT, packet, WD_AUDIO_PACKET_MAX_PAYLOAD_BYTES,
                                     &packet_size))
        {
            __atomic_add_fetch(&stream->stats.encode_failures, 1, __ATOMIC_RELAXED);
            __atomic_store_n(&stream->force_discontinuity, true, __ATOMIC_RELEASE);
        ...
        if (!wd_audio_packetizer_make_packet(&stream->packetizer, pts_samples, WD_AUDIO_FRAME_SAMPLES_DEFAULT, packet_size, &header))
        {
            continue;
        }
        ...
        if (!wd_async_tcp_send_message(stream->tx, stream->tcp_fd, WD_MSG_AUDIO_PACKET, payload, (uint32_t)sizeof(header) + packet_size))
        {
            wd_audio_stream_drop_stale_queue(stream);
            continue;
        }
        ...
        __atomic_add_fetch(&stream->stats.encoded_bytes, packet_size, __ATOMIC_RELAXED);
    }
...
static bool wd_audio_stream_ensure_capture_locked(struct wd_audio_stream* stream) {
    ...
    if (!wd_audio_capture_create(&stream->capture, WD_AUDIO_CHANNELS_MAX, wd_audio_stream_capture, stream))
    {
...
bool wd_audio_stream_start(struct wd_audio_stream* stream, int tcp_fd, uint8_t session_id, uint64_t connection_token, uint64_t audio_epoch,
                           ...
                           uint16_t target_latency_ms) {
    if (!stream || tcp_fd < 0 || session_id == 0 || connection_token == 0 || audio_epoch == 0 || media_clock_id == 0 ||
        media_clock_start_ns == 0 || channels == 0 || channels > WD_AUDIO_CHANNELS_MAX || !wd_audio_stream_available())
    {
        return false;
    }
    ...
    const uint32_t capacity_frames = (WD_AUDIO_SAMPLE_RATE_DEFAULT * WD_AUDIO_CAPTURE_RING_MS) / WD_MSEC_PER_SEC;
    ...
    if (ok)
    {
        ...
        stream->payload_buffer = malloc(sizeof(struct wd_audio_packet_payload_header) + WD_AUDIO_PACKET_MAX_PAYLOAD_BYTES);
        ok                     = stream->pcm_buffer && stream->packet_buffer && stream->payload_buffer;
    }
    if (stream->tx)
    {
        wd_async_tcp_sender_set_max_pending_bytes(stream->tx, stream->tx_max_pending_bytes);
    }
    if (ok)
    {
        wd_audio_packetizer_begin(&stream->packetizer, session_id, connection_token, audio_epoch, media_clock_id);
        ok = wd_audio_stream_send_config(stream);
    }
    if (ok)
    {
        ok                     = pthread_create(&stream->worker, NULL, wd_audio_stream_worker, stream) == 0;
