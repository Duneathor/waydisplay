...
bool wd_audio_packetizer_make_packet(struct wd_audio_packetizer* packetizer,
    ...
    if (!packetizer || !header || packetizer->session_id == 0 ||
        packetizer->connection_token == 0 || packetizer->audio_epoch == 0 ||
        packetizer->media_clock_id == 0 || !wd_audio_frame_samples_is_valid(duration_samples) ||
        data_size == 0 || data_size > WD_AUDIO_PACKET_MAX_PAYLOAD_BYTES)
    {
        return false;
    }
    ...
    if (packetizer->force_discontinuity ||
        (packetizer->have_expected_pts && pts_samples != packetizer->expected_pts_samples))
    {
