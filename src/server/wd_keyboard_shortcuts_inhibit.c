...
static bool inhibitor_should_be_active(struct wd_server* server, struct wlr_keyboard_shortcuts_inhibitor_v1* inhibitor) {
    return server && inhibitor && inhibitor->seat == server->seat && inhibitor->surface && inhibitor->surface == server->focused_surface;
}
...
void wd_keyboard_shortcuts_inhibit_refresh(struct wd_server* server) {
    ...
    wl_list_for_each(state, &server->keyboard_shortcuts_inhibitors, link) {
        ...
        const bool should_activate = inhibitor_should_be_active(server, state->inhibitor);
...
bool wd_keyboard_shortcuts_inhibit_active(struct wd_server* server) {
    ...
    struct wd_keyboard_shortcuts_inhibitor_state* state = NULL;
    wl_list_for_each(state, &server->keyboard_shortcuts_inhibitors, link) {
        if (state->inhibitor && state->inhibitor->active && inhibitor_should_be_active(server, state->inhibitor))
        {
            return true;
        }
    }
...
static void inhibitor_state_destroy(struct wd_keyboard_shortcuts_inhibitor_state* state) {
    if (!state)
    {
        return;
    }
...
static void handle_inhibitor_destroy(struct wl_listener* listener, void* data) {
    ...
    struct wd_keyboard_shortcuts_inhibitor_state* state  = wl_container_of(listener, state, destroy);
...
static void handle_new_keyboard_shortcuts_inhibitor(struct wl_listener* listener, void* data) {
    struct wd_server*                           server    = wl_container_of(listener, server, new_keyboard_shortcuts_inhibitor);
    ...
    if (!state)
    {
        WD_LOG_ERROR("failed to allocate keyboard-shortcuts inhibitor state");
        return;
    }
    ...
    WD_LOG_DEBUG("keyboard-shortcuts-inhibit requested surface=%p seat=%p", (void*)inhibitor->surface, (void*)inhibitor->seat);

    wd_keyboard_shortcuts_inhibit_refresh(server);
}

static void handle_keyboard_shortcuts_inhibit_manager_destroy(struct wl_listener* listener, void* data) {
    (void)data;

    struct wd_server* server = wl_container_of(listener, server, keyboard_shortcuts_inhibit_manager_destroy);

    server->keyboard_shortcuts_inhibit_manager = NULL;

    if (server->new_keyboard_shortcuts_inhibitor.link.prev && server->new_keyboard_shortcuts_inhibitor.link.next)
    {
        wl_list_remove(&server->new_keyboard_shortcuts_inhibitor.link);
        wl_list_init(&server->new_keyboard_shortcuts_inhibitor.link);
    }

    if (server->keyboard_shortcuts_inhibit_manager_destroy.link.prev && server->keyboard_shortcuts_inhibit_manager_destroy.link.next)
    {
...
bool wd_keyboard_shortcuts_inhibit_init(struct wd_server* server) {
    ...
    server->keyboard_shortcuts_inhibit_manager = wlr_keyboard_shortcuts_inhibit_v1_create(server->display);
    if (!server->keyboard_shortcuts_inhibit_manager)
    {
        WD_LOG_ERROR("failed to create keyboard-shortcuts-inhibit manager");
        return false;
    }

    server->new_keyboard_shortcuts_inhibitor.notify = handle_new_keyboard_shortcuts_inhibitor;
    wl_signal_add(&server->keyboard_shortcuts_inhibit_manager->events.new_inhibitor, &server->new_keyboard_shortcuts_inhibitor);
    ...
    WD_LOG_DEBUG("keyboard-shortcuts-inhibit enabled");
