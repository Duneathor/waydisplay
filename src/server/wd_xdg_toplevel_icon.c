...
static struct wd_view *view_from_xdg_toplevel(
    ...
    wl_list_for_each(view, &server->views, link) {
        if (!view->xdg_surface ||
            ...
            view->xdg_surface->toplevel != toplevel) {
            continue;
        }
...
static void handle_set_xdg_toplevel_icon(struct wl_listener *listener,
                                         void *data) {
    struct wd_server *server =
        wl_container_of(listener, server, set_xdg_toplevel_icon);
