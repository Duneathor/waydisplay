...
static void handle_set_xdg_toplevel_icon(struct wl_listener* listener, void* data) {
    ...
    if (!view)
    {
        WD_LOG_DEBUG("WayDisplay: xdg-toplevel-icon for unknown toplevel=%p", (void*)event->toplevel);
        return;
    }
...
bool wd_xdg_toplevel_icon_init(struct wd_server* server) {
    ...
    {
        WD_LOG_ERROR("WayDisplay: failed to create xdg-toplevel-icon manager");
        return false;
    }
