...
void SDLCALL audio_postmix_callback(void* userdata, const SDL_AudioSpec* spec, float* buffer, int buflen) {
    (void)buffer;
    ...
    const uint64_t bytes_per_frame  = sizeof(float) * static_cast<uint64_t>(spec->channels);
    const uint64_t frames           = static_cast<uint64_t>(buflen) / bytes_per_frame;
    const uint32_t sample_rate      = playback->config.sample_rate;
    const uint64_t mixed_samples_fp = client_audio_frames_to_samples_fp(frames, static_cast<uint32_t>(spec->freq), sample_rate);
    const uint64_t buffer_samples   = mixed_samples_fp >> CLIENT_AUDIO_CLOCK_FRACTION_BITS;
    playback->device_buffer_samples.store(buffer_samples, std::memory_order_release);
    const uint64_t mixed_total_fp =
        playback->device_mixed_samples_fp.fetch_add(mixed_samples_fp, std::memory_order_acq_rel) + mixed_samples_fp;
    const uint64_t limit = playback->device_clock_limit_samples.load(std::memory_order_acquire);
    if (limit != 0 && (mixed_total_fp >> CLIENT_AUDIO_CLOCK_FRACTION_BITS) >= limit + buffer_samples)
    {
...
void publish_device_clock_limit_locked(ClientAudioPlayback* playback) {
    uint64_t limit = 0;
    if (playback && playback->have_playback_start_pts && playback->submitted_end_pts >= playback->playback_start_pts)
    {
        limit = playback->submitted_end_pts - playback->playback_start_pts;
    }
    if (playback)
    {
        playback->device_clock_limit_samples.store(limit, std::memory_order_release);
    }
}
...
bool client_audio_playback_configure(ClientAudioPlayback* playback, const wd_audio_config_payload& config, uint16_t target_latency_ms) {
    if (!playback || !wd_audio_config_payload_is_valid(&config, sizeof(config)) || target_latency_ms < WD_AUDIO_TARGET_LATENCY_MS_MIN ||
    ...
    int error         = OPUS_OK;
    playback->decoder = opus_decoder_create(static_cast<opus_int32>(config.sample_rate), config.channels, &error);
    ...
    playback->stream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &spec, nullptr, nullptr);
    if (!playback->stream)
    {
    ...
    if (playback->device_id == 0 || !SDL_GetAudioDeviceFormat(playback->device_id, &device_spec, &device_buffer_frames))
    {
        WD_LOG_ERROR("failed to configure SDL audio presentation clock: %s", SDL_GetError());
        destroy_stream_locked(playback);
        return false;
    }
    playback->device_buffer_samples.store(
        client_audio_frames_to_samples_fp(device_buffer_frames > 0 ? static_cast<uint64_t>(device_buffer_frames) : 0,
...
bool client_audio_playback_handle_packet(ClientAudioPlayback* playback, const uint8_t* payload, uint32_t payload_size) {
    ...
    if (!playback->configured || header.session_id != playback->config.session_id ||
        header.connection_token != playback->config.connection_token || header.audio_epoch != playback->config.audio_epoch ||
        header.media_clock_id != playback->config.media_clock_id)
    {
        return false;
    }

    const bool sequence_gap       = playback->expected_sequence != 0 && header.sequence != playback->expected_sequence;
    const bool pts_gap            = playback->have_expected_pts && header.pts_samples != playback->expected_pts_samples;
    const bool wire_discontinuity = (header.flags & WD_AUDIO_PACKET_DISCONTINUITY) != 0;
    ...
    return false;
#else
    if (header.duration_samples != playback->config.frame_samples || UINT64_MAX - header.pts_samples < header.duration_samples)
    {
        return false;
    }

    const uint8_t* packet  = payload + sizeof(header);
    const int      decoded = opus_decode_float(playback->decoder, packet, static_cast<opus_int32>(header.data_size),
    ...
    if (queued_frames > 0)
    {
        ...
        const int    byte_count = queued_frames * playback->config.channels * static_cast<int>(sizeof(float));
...
bool client_audio_playback_video_gate(ClientAudioPlayback* playback, uint64_t now_ns, uint32_t* hold_age_ms, bool* timed_out) {
    ...
    return decision == WD_CLIENT_AUDIO_STARTUP_HOLD;
}
