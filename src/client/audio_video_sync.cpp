...
ClientVideoAudioSyncPlan client_video_audio_sync_plan(uint64_t video_pts_usec, uint64_t audio_playhead_samples, uint32_t sample_rate) {
    ...
    const uint64_t clamped_delta = std::min<uint64_t>(positive_delta, static_cast<uint64_t>(std::numeric_limits<int64_t>::max()));
    plan.delta_samples =
        video_samples >= audio_playhead_samples ? static_cast<int64_t>(clamped_delta) : -static_cast<int64_t>(clamped_delta);
    ...
    if (video_samples > audio_playhead_samples && video_samples - audio_playhead_samples > early)
    {
        ...
        const uint64_t whole_ms          = whole_seconds > UINT32_MAX / WD_MSEC_PER_SEC ? UINT32_MAX : whole_seconds * WD_MSEC_PER_SEC;
        const uint64_t remainder_ms      = (remainder_samples * WD_MSEC_PER_SEC + sample_rate - 1u) / sample_rate;
        const uint64_t wait_ms           = std::min<uint64_t>(WD_CLIENT_VIDEO_AUDIO_MAX_RETRY_MS, whole_ms + remainder_ms);
...
ClientVideoAudioSyncDecision client_video_audio_sync_decide(uint64_t video_pts_usec, uint64_t audio_playhead_samples,
    ...
    return client_video_audio_sync_plan(video_pts_usec, audio_playhead_samples, sample_rate).decision;
}
