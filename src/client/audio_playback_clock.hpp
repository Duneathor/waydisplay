...
constexpr uint64_t CLIENT_AUDIO_CLOCK_FRACTION_ONE =
    uint64_t{1} << CLIENT_AUDIO_CLOCK_FRACTION_BITS;

inline uint64_t client_audio_frames_to_samples_fp(uint64_t frames,
                                                  uint32_t frame_rate,
    ...
    const uint64_t fractional_samples_fp =
        (fractional_numerator * CLIENT_AUDIO_CLOCK_FRACTION_ONE) / frame_rate;
    return whole_samples * CLIENT_AUDIO_CLOCK_FRACTION_ONE +
           fractional_samples_fp;
}

inline uint64_t client_audio_device_playhead(uint64_t start_pts,
                                             uint64_t submitted_end_pts,
                                             uint64_t mixed_samples_fp,
                                             uint64_t device_buffer_samples) {
    const uint64_t mixed_samples =
        mixed_samples_fp >> CLIENT_AUDIO_CLOCK_FRACTION_BITS;
    const uint64_t presented_samples = mixed_samples > device_buffer_samples
                                           ? mixed_samples - device_buffer_samples
                                           : 0;
    const uint64_t available_samples = submitted_end_pts >= start_pts
                                           ? submitted_end_pts - start_pts
                                           : 0;
    return start_pts + std::min(presented_samples, available_samples);
}

inline bool client_audio_device_consumed(uint64_t start_pts,
                                         uint64_t submitted_end_pts,
                                         uint64_t mixed_samples_fp,
                                         uint64_t device_buffer_samples) {
