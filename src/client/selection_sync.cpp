...
std::string& sent_text(ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_sent_primary : state.last_sent_clipboard;
}

const std::string& sent_text(const ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_sent_primary : state.last_sent_clipboard;
}

bool& sent_valid(ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_sent_primary_valid : state.last_sent_clipboard_valid;
}
...
std::string& applied_text(ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_applied_primary : state.last_applied_clipboard;
}

const std::string& applied_text(const ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_applied_primary : state.last_applied_clipboard;
}

bool& applied_valid(ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_applied_primary_valid : state.last_applied_clipboard_valid;
}

bool applied_valid(const ClientSelectionSyncState& state, ClientSelectionKind kind) {
    return kind == ClientSelectionKind::Primary ? state.last_applied_primary_valid : state.last_applied_clipboard_valid;
}
